  ...
  <head>
	...
	<script>
		function sell() {
			...
			alert("We are not buying anymore crates right now! We have an excess of crates in stock and need to sell what we have, come back later.");
		}
		function buy() {
		...
		function submitSell() {
			...
			var count2 = document.getElementById("crateCounter2").value;
			var ppURL = document.getElementById("paypalURL_S").value;
			var x = confirm("Are you sure you want to sell us " + count + " #1 crates and " + count2 +" #2 crates?");
			if(x == true && url != "" && ppURL != "") {
				$.ajax({
				...
				<ul class="accordion-group" id="accordion">
					<li style="background-image: url('img/1.jpg');" onclick="sell()">
						...
						<section class="hidden-xs">
							<article>
								<p>This feature will be reactivated very soon! We got caught up in a crate price drop over the week with far too many crates in stock.</p>
							</article>
						</section>
					</li>
					...
					<li style="background-image: url('img/4.jpg');" onclick="keys()">
						...
						<section class="hidden-xs">
							<article>
								<p>Hate that annoying $5.00 minimum Steam purchase when all you really need is 1-2 keys?</p>
								<p>Guaranteed Trade within 24 hours. We use real people, not bots, that sadly need to sleep.</p>
							</article>
						</section>
					</li>
					...
					<div class="row">
						...
						<div class="col-md-4 col-sm-6 col-xs-12">
							<div class="product-box">
								...
								<h3 class="text-center">Referral System</h3>
								<p>The Referral System is something we want to implement so that we can give back to loyal customers who support RLCTrade.com- rewarding them with free keys and crates!</p>
							</div>
						</div>
					</div>
				</div>
				...
				<footer class="footer-inner">
					<div class="container">
						<div class="row">
							...
							<div class="col-md-3">
								<a class="twitter-timeline" href="https://twitter.com/RLCTrade">Tweets by RLCTrade</a> <script async src="//platform.twitter.com/widgets.js" charset="utf-8"></script>
							</div>
						</div>
					</div>
				</footer>
										...
										<div class="form-group">
											<div class="input-group">
												...
												<select id="crateCounter" required>
													<option value="0">Champion Crate #1 to Sell (Max 50)</option>
													<?php
														$pricePer = 1.40;
												...
												<select id="crateCounter2" required>
													<option value="0">Champion Crate #2 to Sell (Max 50)</option>
													<?php
														$pricePer2 = 1.15;
														setlocale(LC_MONETARY, 'en_US');
														for ($i = 0; $i <= 50; $i++) {
															$totalPrice2 = $pricePer2 * $i;
															echo '<option value="'.$i.'">'.$i.' - '.money_format('%i', $totalPrice2).'</option>';
														}
													?>
												</select>
												<div class="input-group-addon-holder">
			...
			<aside id="FAQ" class="modal fade">
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">
							<div class="row">
								<div id="faq" class="col-md-8 col-md-offset-2 col-sm-12">
									...
									<hr />
									<p>Q - If I sell crates but do not use PayPal; can you exchange the value of the crates for something else?</p>
									<p>A - Of course! We have real employees here at rlctrade.com meaning we don't use bots. If you want us to gift you a game on steam- as long as the value of the crates equals the price of the game, we would be happy to oblige. (Possibly expanding to Amazon in the future.)<p/>
								</div>
							</div>
						</div>
					</div>
